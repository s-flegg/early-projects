...
# TODO: simple animation background, icons for producers showing amount, upgrades, and buybutton with cost
...
class Icons(pygame.sprite.Sprite):
    ...
    def __init__(self, surface, centre_pos, bg_colour, name, producer, data):
        ...
        self.text_name = gb.Text(
            ...
            post_var_text=None,
            var_func=None,  # FIXME: in gb change Text to CentredText and create StartPosText, using CentredText as
                            #       masterclass, changing centre using width and height
                            #       then use centred text here
        )
        ...
        self.buttons = RenderUpdates(self.button_buy, self.text_amount, self.text_upgrade)
...
class Data:
    ...
    def load_specific_save_data(self, save):
        ...
        for i in range(10):
            self.producers_list[i].initialise(upgrades_list[i], amounts_list[i], costs_list[i])
...
class Initialise:
    ...
    def create_producers(self):
        producer_0 = DefaultProducer(produces=self.data.currency_holder,
                                     cost=1,
                                     game=self.data)
        ...
        for i in range(1, 10):
            self.data.producers_list.append(
                DefaultProducer(
                    produces=self.data.producers_list[int(i - 1)], cost=pow(10, i), game=self.data)
            )
        return
...
class Menu:
    ...
    def screen_new(self):
        ...
        buttons = RenderUpdates(button_save_1, button_save_2, button_save_3, button_back)
